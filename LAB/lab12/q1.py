...
class MaxHeap:
    ...
    def max(self):
        ...
        if self._size == 0:
            raise KeyError('heap: max. empty heap')
        return self._data[0]
    ...
    def remove_max(self):
        ...
        if self._size == 0:
            raise KeyError('heap: remove_max. empty heap')
        result = self._data[0]
        self._size -= 1
    ...
    def down_heap(self, index):
        ...
        while not self._is_leaf(index):
            max_child = self._left(index)
            if self._has_right(index):
                if self._data[self._right(index)] > self._data[max_child]:
                    max_child = self._right(index)
            if self._data[max_child] > self._data[index]:
                self._swap(max_child, index)
                index = max_child
            else:
                break
...
def k_smallest_elements(lst, k):
    ...
    output_heap = MaxHeap()
    for i in range(k):
        output_heap.add(lst[i])
    for i in range(k, len(lst)):
        if lst[i] < output_heap.max():
            output_heap._data[0] = lst[i]
            output_heap.down_heap(0)
    return output_heap._data


if __name__ == '__main__':
    print('If k is smaller than length of list')
    print(k_smallest_elements([3, 9, 2, 7, 1, 6, 5, 8], 4))
    print('If k is larger than length of list')
    print(k_smallest_elements([3, 9, 2, 7, 1, 6, 5, 8], 10))
